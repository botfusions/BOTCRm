...
## 2026-06-03 - Opportunity Details Optimization and Local Fallback Resilience
**Learning:** Hoisting static data structures (like progress stages and list contents) outside of heavy detail-view components prevents CPU churn and garbage collection pauses from redundant array re-allocations on every render cycle. Additionally, ensuring that SDK clients (like Supabase) do not throw synchronously during initial module evaluation allows the React application bundle to load successfully even when environment variables are omitted, while route-level fallbacks prevent visual hanging.
**Action:** Always hoist invariant arrays and configuration objects outside component definitions. Wrap detail and nested list item views in `React.memo` to shield them from parent rendering cascades, and use dummy fallbacks for critical client credentials during local development.
