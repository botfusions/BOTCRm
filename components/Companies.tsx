 ...
 * Extracted CompanyCard into a memoized component to prevent unnecessary re-renders
 * during parent state changes (e.g., search term updates or view toggles).
 *
 * Native 'loading="lazy"' is applied to the company logo for better scroll performance.
 */
...
/**
 * ⚡ PERFORMANCE OPTIMIZATION:
 * Memoized table row component to minimize DOM updates during list filtering.
 */
    ...
    <tr className="hover:bg-indigo-500/5 transition-colors group">
        <td className="p-5">
            <div className="flex items-center gap-3">
                <img src={company.logoUrl} className="w-8 h-8 rounded-lg" loading="lazy" />
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className={`divide-y ${darkMode ? 'divide-slate-800' : 'divide-slate-100'}`}>
                                {filteredCompanies.map(c => (
                                    <CompanyRow key={c.id} company={c} textMain={textMain} />
                                ))}
