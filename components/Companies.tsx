...
// ⚡ PERFORMANCE OPTIMIZATION: Extract list items into memoized components to prevent
// unnecessary re-renders of the entire list when unrelated state (like search term) changes.
const CompanyGridItem = React.memo(({ company, darkMode }: { company: Company; darkMode: boolean }) => {
...
const CompanyTableRow = React.memo(({ company, darkMode }: { company: Company; darkMode: boolean }) => {
    const textMain = darkMode ? 'text-white' : 'text-slate-900';
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {filteredCompanies.map(company => (
                            <CompanyGridItem key={company.id} company={company} darkMode={darkMode} />
                        ))}
                    </div>
                ) : (
                    <div className={`rounded-3xl border overflow-hidden ${bgCard}`}>
                        <table className="w-full text-left">
                            ...
                            <tbody className={`divide-y ${darkMode ? 'divide-slate-800' : 'divide-slate-100'}`}>
                                {filteredCompanies.map(company => (
                                    <CompanyTableRow key={company.id} company={company} darkMode={darkMode} />
                                ))}
