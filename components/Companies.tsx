...
const Companies: React.FC<CompaniesProps> = ({ darkMode }) => {
    ...
    // ⚡ PERFORMANCE OPTIMIZATION: Memoize filtered companies to optimize performance during search and re-renders
    ...
    const handleSetViewType = useCallback((type: 'grid' | 'table') => {
        setViewType(type);
    }, []);

    const handleSetIsAddModalOpen = useCallback((isOpen: boolean) => {
            ...
            <header className="flex justify-between items-end mb-8 shrink-0">
                ...
                <div className="flex gap-3">
                    <div className={`flex p-1 rounded-xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                        <button onClick={() => handleSetViewType('grid')} className={`p-2 rounded-lg transition-all ${viewType === 'grid' ? 'bg-indigo-600 text-white' : textSub}`}><LayoutGrid className="w-4 h-4"/></button>
                        <button onClick={() => handleSetViewType('table')} className={`p-2 rounded-lg transition-all ${viewType === 'table' ? 'bg-indigo-600 text-white' : textSub}`}><List className="w-4 h-4"/></button>
                    </div>
                    <button 
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
                    <div className={`w-full max-w-md rounded-[3rem] border shadow-2xl overflow-hidden ${bgCard}`}>
                        <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-indigo-600/5">
                            ...
                            <button onClick={() => handleSetIsAddModalOpen(false)} className={textSub}><X className="w-6 h-6"/></button>
                        </div>
