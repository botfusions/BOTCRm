...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: CompanyCard
 * Extracted into a standalone component and wrapped in React.memo to prevent
 * unnecessary re-renders when the parent component state changes (e.g., during search).
 * Added loading="lazy" to the company logo to improve initial load performance.
 */
    ...
    <div className={`p-6 rounded-3xl border transition-all hover:border-indigo-500/30 group ${bgCard} hover:shadow-xl hover:scale-[1.01]`}>
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
            <span className={`px-4 py-1 rounded-xl text-[9px] font-black uppercase border shadow-sm
             ...
             <button className={`p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 ${textSub}`} aria-label="More options"><MoreVertical className="w-5 h-5" /></button>
        </div>
    </div>
));

/**
 * ⚡ PERFORMANCE OPTIMIZATION: CompanyRow
 * Extracted into a standalone component and wrapped in React.memo to prevent
 * full table re-renders when individual search terms change or views toggle.
 * Added loading="lazy" to images for better scrolling performance.
 */
