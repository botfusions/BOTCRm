...
}> = React.memo(({ contact, textMain, onView }) => (
...
const ContactCard: React.FC<{
...
}> = React.memo(({ contact, darkMode, bgCard, textMain, textSub, isMenuOpen, onOpenMenu, onView, onDelete }) => (
    <div className={`group relative rounded-[2.5rem] border transition-all duration-300 hover:shadow-2xl hover:scale-[1.01] overflow-hidden ${bgCard} hover:border-indigo-500/40`}>
        <div className="h-28 bg-indigo-600 relative overflow-hidden">
            ...
            <button
                onClick={() => onOpenMenu(isMenuOpen ? null : contact.id)}
            ...
            {isMenuOpen && (
                <div className={`absolute right-4 top-14 w-32 rounded-xl border shadow-xl z-30 overflow-hidden ${bgCard} animate-in fade-in slide-in-from-top-2`}>
                    <button onClick={() => {onView(contact); onOpenMenu(null);}} className={`w-full text-left px-4 py-2.5 text-xs hover:bg-indigo-50 dark:hover:bg-indigo-900/30 flex items-center gap-2 ${textMain}`}><Eye className="w-3.5 h-3.5"/> Gör</button>
...
const Contacts: React.FC<ContactsProps> = ({ darkMode, language = 'TR' }) => {
    ...
    }, [setContacts, setErrorType, setLoading]);
    ...
    }, [newContact, setContacts, setIsAddModalOpen, setNewContact]);
