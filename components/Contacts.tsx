...
// ⚡ PERFORMANCE OPTIMIZATION: Extract list items into memoized components to prevent redundant re-renders
...
}> = React.memo(({ contact, darkMode, bgCard, textMain, textSub, isMenuOpen, onToggleMenu, onSelect, onDelete }) => (
...
const ContactRow: React.FC<{
    ...
    textMain: string;
    onSelect: (contact: Contact) => void;
}> = React.memo(({ contact, textMain, onSelect }) => (
    <tr className="hover:bg-indigo-500/5 transition-colors group">
        <td className="p-5">
            <div className="flex items-center gap-4">
                <img src={contact.avatarUrl} className="w-10 h-10 rounded-xl" loading="lazy" alt="" />
...
const Contacts: React.FC<ContactsProps> = ({ darkMode, language = 'TR' }) => {
    ...
    // ⚡ PERFORMANCE OPTIMIZATION: Memoize callbacks to prevent unnecessary re-renders of memoized list items
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
                    <div className={`w-full max-w-lg rounded-[3rem] border shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 ${bgCard}`}>
                        <div className="relative h-36 bg-indigo-600">
                            <div className="absolute inset-0 bg-gradient-to-r from-indigo-700 to-indigo-500 opacity-90"></div>
                             <button onClick={() => handleSelectContact(null)} className="absolute top-6 right-6 p-2 bg-black/20 text-white rounded-full hover:bg-black/40 backdrop-blur-md transition-transform hover:rotate-90"><X className="w-5 h-5"/></button>
                        </div>
