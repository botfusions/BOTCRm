...
}> = React.memo(({ contact, darkMode, bgCard, textMain, textSub, isMenuOpen, onMenuToggle, onSelect, onDelete }) => (
        ...
        <div className="h-28 bg-indigo-600 relative overflow-hidden">
            ...
            {isMenuOpen && (
                <div className={`absolute right-4 top-14 w-32 rounded-xl border shadow-xl z-30 overflow-hidden ${bgCard} animate-in fade-in slide-in-from-top-2`}>
                    <button onClick={() => {onSelect(contact); onMenuToggle(null);}} className={`w-full text-left px-4 py-2.5 text-xs hover:bg-indigo-50 dark:hover:bg-indigo-900/30 flex items-center gap-2 ${textMain}`}><Eye className="w-3.5 h-3.5"/> Gör</button>
