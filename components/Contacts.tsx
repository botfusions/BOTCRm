...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: ContactCard
 * Extracted into a standalone component to isolate re-renders.
 * Uses React.memo to prevent unnecessary updates during parent state changes (like search input).
 */
...
const ContactCard: React.FC<ContactCardProps> = React.memo(({
    contact, darkMode, textMain, textSub, bgCard, isMenuOpen, onMenuToggle, onView, onDelete
}) => {
    return (
        <div className={`group relative rounded-[2.5rem] border transition-all duration-300 hover:shadow-2xl hover:scale-[1.01] overflow-hidden ${bgCard} hover:border-indigo-500/40`}>
            <div className="h-28 bg-indigo-600 relative overflow-hidden">
                ...
                <button
                    onClick={() => onMenuToggle(isMenuOpen ? null : contact.id)}
                ...
                {isMenuOpen && (
                    <div className={`absolute right-4 top-14 w-32 rounded-xl border shadow-xl z-30 overflow-hidden ${bgCard} animate-in fade-in slide-in-from-top-2`}>
                        <button onClick={() => {onView(contact); onMenuToggle(null);}} className={`w-full text-left px-4 py-2.5 text-xs hover:bg-indigo-50 dark:hover:bg-indigo-900/30 flex items-center gap-2 ${textMain}`}><Eye className="w-3.5 h-3.5"/> Gör</button>
            ...
            <div className="px-8 pb-10 -mt-12 relative z-10">
                <div className="flex items-end gap-5 mb-6">
                    <img src={contact.avatarUrl} className="w-24 h-24 rounded-3xl border-4 border-white dark:border-slate-900 shadow-2xl bg-slate-100" loading="lazy" alt="" />
                ...
                <div className="flex gap-3">
                    <button onClick={() => onView(contact)} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl text-[11px] font-black tracking-[0.15em] hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/20 uppercase">Mesaj Gönder</button>
...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: ContactRow
 * Memoized table row component to prevent full table re-renders during search.
 */
...
const ContactRow: React.FC<ContactRowProps> = React.memo(({ contact, textMain, onView }) => {
    return (
        <tr className="hover:bg-indigo-500/5 transition-colors group">
            <td className="p-5">
                <div className="flex items-center gap-4">
                    <img src={contact.avatarUrl} className="w-10 h-10 rounded-xl" loading="lazy" />
            ...
            <td className="p-5 text-right">
                <button onClick={() => onView(contact)} className="p-2 text-slate-400 hover:text-indigo-500 opacity-0 group-hover:opacity-100 transition-all"><ExternalLink className="w-4 h-4"/></button>
            </td>
        </tr>
    );
});
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        {filteredContacts.map(contact => (
                            <ContactCard
                                ...
                                textSub={textSub}
                                bgCard={bgCard}
                                isMenuOpen={openMenuId === contact.id}
