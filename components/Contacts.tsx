...
// ⚡ PERFORMANCE OPTIMIZATION: Memoized ContactCard to prevent redundant re-renders of list items
const ContactCard: React.FC<{
    ...
    textMain: string;
    textSub: string;
}> = React.memo(({ contact, isMenuOpen, onToggleMenu, onView, onDelete, darkMode, bgCard, textMain, textSub }) => (
        ...
        <div className="h-28 bg-indigo-600 relative overflow-hidden">
            ...
            {isMenuOpen && (
                <div className={`absolute right-4 top-14 w-32 rounded-xl border shadow-xl z-30 overflow-hidden ${bgCard} animate-in fade-in slide-in-from-top-2`}>
                    <button onClick={() => onView(contact)} className={`w-full text-left px-4 py-2.5 text-xs hover:bg-indigo-50 dark:hover:bg-indigo-900/30 flex items-center gap-2 ${textMain}`}><Eye className="w-3.5 h-3.5"/> Gör</button>
...
// ⚡ PERFORMANCE OPTIMIZATION: Memoized ContactRow for table view efficiency
const ContactRow: React.FC<{
    contact: Contact;
    onView: (contact: Contact) => void;
    textMain: string;
}> = React.memo(({ contact, onView, textMain }) => (
    <tr className="hover:bg-indigo-500/5 transition-colors group">
        <td className="p-5">
            <div className="flex items-center gap-4">
                <img src={contact.avatarUrl} className="w-10 h-10 rounded-xl" alt="" loading="lazy" />
...
const Contacts: React.FC<ContactsProps> = ({ darkMode, language = 'TR' }) => {
    ...
    // ⚡ PERFORMANCE OPTIMIZATION: Stabilized callbacks to prevent unnecessary re-renders of memoized items
    ...
    const handleToggleMenu = React.useCallback((id: string) => {
