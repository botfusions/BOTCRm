...
// ⚡ PERFORMANCE OPTIMIZATION: Extract AI Capture Panel into a memoized component to isolate typing states and avoid re-rendering the parent lead list on every key stroke
const AICapturePanel = React.memo(({
  darkMode,
  bgCard,
  onAddLead,
  onError
  ...
  darkMode: boolean;
  bgCard: string;
  onAddLead: (lead: Omit<Lead, 'id'>) => Promise<boolean>;
  onError: () => void;
}) => {
  ...
  const handleQuickSave = async (e: React.FormEvent) => {
    ...
    if (!newLead.fullName || !newLead.email || !newLead.phone) {
      alert('Lütfen en az Ad, E-posta ve Telefon bilgilerini doldurun.');
      return;
    }
...
// ⚡ PERFORMANCE OPTIMIZATION: Extract Add Lead Modal into a memoized component to prevent full-page re-renders during manual insertion
const AddLeadModal = React.memo(({
  darkMode,
  bgCard,
  textMain,
  textSub,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className={`w-full max-w-lg rounded-[3rem] border shadow-2xl p-10 ${bgCard}`}>
        <div className="flex justify-between items-center mb-8">
          <h2 className={`text-xl font-black uppercase tracking-widest ${textMain}`}>{language === 'TR' ? 'Yeni Kayıt' : 'New Record'}</h2>
        ...
        {duplicateError && (
          <div className="mb-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs font-bold text-amber-500">
            {language === 'TR' ? 'Bu e-posta zaten kullanımda!' : 'This email is already in use!'}
          </div>
        )}
        <form onSubmit={handleQuickSave} className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">{language === 'TR' ? 'Müşteri İsmi' : 'Client Name'}</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">{language === 'TR' ? 'Telefon' : 'Phone'}</label>
          ...
          <button type="submit" className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-xs tracking-[0.2em] shadow-2xl shadow-indigo-600/30 hover:bg-indigo-700 transition-all uppercase">
            {language === 'TR' ? 'Veritabanına İşle' : 'Commit to Database'}
          </button>
        </form>
      </div>
    </div>
...
const Leads: React.FC<LeadsProps> = ({ darkMode, language = 'TR' }) => {
  ...
  // ⚡ PERFORMANCE OPTIMIZATION: Stabilize onAddLead callback using useCallback to prevent children (AICapturePanel, AddLeadModal) from re-rendering
  ...
  const bgCard = useMemo(() => darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200', [darkMode]);
  const textMain = useMemo(() => darkMode ? 'text-white' : 'text-slate-900', [darkMode]);
  const textSub = useMemo(() => darkMode ? 'text-slate-400' : 'text-slate-500', [darkMode]);
