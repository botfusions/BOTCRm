...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: Hoist static configuration data outside component scope
 * to avoid recreating arrays and objects on every render.
 */
const STAGES = [
...
interface ContactCardProps {
  contact: typeof CONTACTS_DATA[0];
  bgCard: string;
  textMain: string;
  textSub: string;
}
...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: Memoized ContactCard component to avoid unneeded re-renders
 * and native lazy-loading on images (`loading="lazy"`) to optimize asset load performance.
 */
const ContactCard: React.FC<ContactCardProps> = React.memo(({ contact, bgCard, textMain, textSub }) => (
  <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
    <div className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded mb-3 ${contact.roleClass}`}>
      {contact.role}
    </div>
...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: Wrapped OpportunityDetails in React.memo to prevent
 * redundant top-level component re-renders when parent state updates.
 */
