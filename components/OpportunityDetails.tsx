...
/*
  ...
  1. Hoisted static STAGES and CONTACTS_DATA arrays outside the component scope to avoid allocations on every render cycle.
  2. Extracted ContactCard into a React.memo'd sub-component to isolate item-level renders and reduce DOM recreation cost.
  3. Added loading="lazy" to profile image assets to defer offscreen image fetching.
  4. Wrapped OpportunityDetails in React.memo to prevent unnecessary parent re-renders from cascading down.
*/
...
const CONTACTS_DATA: ContactItem[] = [
  {
    ...
    roleColorClass: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300',
    name: 'Jane Doe',
  ...
  },
  {
    id: '2',
    role: 'Key Influencer',
    roleColorClass: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
    name: 'John Smith',
    title: 'IT Director',
...
const ContactCard: React.FC<ContactCardProps> = React.memo(({ contact, bgCard, textMain, textSub }) => (
  <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
    <div className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded mb-3 ${contact.roleColorClass}`}>
      {contact.role}
    </div>
...
const OpportunityDetails: React.FC<OpportunityDetailsProps> = React.memo(({ darkMode, onBack }) => {
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="pb-6">
                                <div className="flex items-center gap-2 mb-1">
                                    <img src="https://picsum.photos/100/100?random=10" className="w-5 h-5 rounded-full" alt="John" loading="lazy" />
