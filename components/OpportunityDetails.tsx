...
// ⚡ PERFORMANCE OPTIMIZATION: Hoist static data outside component scope to prevent re-allocation on every render.
const STAGES = [
...
const CONTACTS_DATA: Contact[] = [
  {
    ...
    roleColor: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300',
  ...
  },
  {
    ...
    title: 'IT Director',
    role: 'Key Influencer',
    roleColor: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
...
// ⚡ PERFORMANCE OPTIMIZATION: Extract ContactCard into a React.memo subcomponent with lazy loading on avatars
const ContactCard: React.FC<{ contact: Contact; bgCard: string; textMain: string; textSub: string }> = React.memo(
  ({ contact, bgCard, textMain, textSub }) => (
    <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
      <div className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded mb-3 ${contact.roleColor}`}>
        {contact.role}
      </div>
...
// ⚡ PERFORMANCE OPTIMIZATION: Wrap OpportunityDetails in React.memo to prevent unnecessary re-renders when parent state updates.
