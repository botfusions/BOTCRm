...
// to prevent object re-allocation and garbage collection overhead on every render pass.
const STAGES = [
...
const CONTACTS_DATA = [
  {
    ...
    typeClass: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300',
    name: 'Jane Doe',
  ...
  {
    type: 'Key Influencer',
    typeClass: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
    name: 'John Smith',
    role: 'IT Director',
...
// ⚡ PERFORMANCE OPTIMIZATION: Extract and memoize repetitive, complex sub-components.
// Using React.memo ensures individual contact cards are only updated when their specific properties change.
const ContactCard: React.FC<{
  ...
  return (
    <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
      <div className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded mb-3 ${contact.typeClass}`}>
        {contact.type}
      </div>
...
// ⚡ PERFORMANCE OPTIMIZATION: Memoized the primary OpportunityDetails component
// to prevent useless panel tree reconciliation if only unrelated outer states trigger a parent update.
