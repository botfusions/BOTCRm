...
const CONTACTS_DATA: ContactData[] = [
  {
    ...
    badgeBg: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300',
    name: 'Jane Doe',
...
/* ⚡ PERFORMANCE OPTIMIZATION: Extract ContactCard into a memoized component
   and lazy load avatars to reduce DOM updates and initial image resource overhead. */
const ContactCard: React.FC<{ contact: ContactData; bgCard: string; textMain: string; textSub: string }> = React.memo(
  ({ contact, bgCard, textMain, textSub }) => {
    return (
      <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
        <div className={`inline-block px-2 py-0.5 ${contact.badgeBg} text-[10px] font-bold uppercase tracking-wider rounded mb-3`}>
          {contact.role}
        </div>
        <div className="flex items-center gap-3 mb-4">
          <img src={contact.avatar} loading="lazy" className="w-10 h-10 rounded-full" alt={contact.name} />
          <div>
...
/* ⚡ PERFORMANCE OPTIMIZATION: Wrap OpportunityDetails in React.memo
   to prevent re-rendering when parent component re-renders unless props change. */
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="pb-6">
                                <div className="flex items-center gap-2 mb-1">
                                    <img src="https://picsum.photos/100/100?random=10" loading="lazy" className="w-5 h-5 rounded-full" alt="John" />
