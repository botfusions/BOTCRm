 ...
 * - Hoisted static configuration arrays (STAGES and CONTACTS_DATA) outside of the component
 *   to avoid re-creating them on every render pass.
 * - Extracted repeating Contact Cards into a standalone, memoized `ContactCard` component
 *   to prevent redundant tree-reconciliation and layout thrashing.
 * - Used native `loading="lazy"` on profile images to reduce upfront assets fetch overhead.
 * - Wrapped `OpportunityDetails` in `React.memo` to isolate it from unnecessary re-renders
 *   when parent components (like `Leads.tsx`) experience state changes.
 */
...
const CONTACTS_DATA = [
  {
    ...
    roleTypeColor: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300',
    name: 'Jane Doe',
  ...
  {
    roleType: 'Key Influencer',
    roleTypeColor: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
    name: 'John Smith',
    title: 'IT Director',
...
const ContactCard: React.FC<ContactCardProps> = React.memo(({
  ...
  return (
    <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
       <div className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded mb-3 ${roleTypeColor}`}>
          {roleType}
       </div>
       <div className="flex items-center gap-3 mb-4">
          <img src={image} className="w-10 h-10 rounded-full" loading="lazy" alt={name} />
          <div>
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="pb-6">
                                <div className="flex items-center gap-2 mb-1">
                                    <img src="https://picsum.photos/100/100?random=10" className="w-5 h-5 rounded-full" loading="lazy" />
                ...
                <div>
                    ...
                    {CONTACTS_DATA.map((contact, idx) => (
                      <ContactCard
                        ...
                        roleTypeColor={contact.roleTypeColor}
                        name={contact.name}
                        title={contact.title}
                        email={contact.email}
                        phone={contact.phone}
...
export default React.memo(OpportunityDetails);
