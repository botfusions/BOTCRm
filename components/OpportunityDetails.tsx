...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: Hoisted static content arrays outside the component body
 * to prevent redundant object and array recreation on every single render.
 */
const STAGES = [
...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: Extracted and wrapped ContactCard inside React.memo
 * to isolate re-renders of list items and prevent full list recalculation.
 * Profile images are also lazy-loaded natively via loading="lazy".
 */
const ContactCard: React.FC<{
  ...
  return (
    <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
      <div className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded mb-3 ${contact.badgeClass}`}>
        {contact.badge}
      </div>
...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: Wrapped OpportunityDetails component in React.memo
 * to avoid expensive re-renders when parent views (like Leads) update state (such as input forms or text-area captures).
 */
