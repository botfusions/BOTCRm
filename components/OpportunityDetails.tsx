...
// ⚡ PERFORMANCE OPTIMIZATION: Hoisted static stage definitions outside component scope
const STAGES = [
...
// ⚡ PERFORMANCE OPTIMIZATION: Hoisted static contact data outside component scope
const CONTACTS_DATA = [
  {
    id: 1,
    role: 'Primary Contact',
    roleClass: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300',
    name: 'Jane Doe',
  ...
  },
  {
    id: 2,
    role: 'Key Influencer',
    roleClass: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
    name: 'John Smith',
    title: 'IT Director',
...
// ⚡ PERFORMANCE OPTIMIZATION: Memoized contact card sub-component with lazy loading images
const ContactCard: React.FC<ContactCardProps> = React.memo(({
  ...
  <div className={`rounded-xl border p-4 shadow-sm mb-3 ${bgCard}`}>
    <div className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded mb-3 ${roleClass}`}>
      {role}
    </div>
    <div className="flex items-center gap-3 mb-4">
      <img src={avatar} alt={name} loading="lazy" className="w-10 h-10 rounded-full" />
      <div>
        <p className={`text-sm font-bold ${textMain}`}>{name}</p>
        <p className={`text-xs ${textSub}`}>{title}</p>
      </div>
 ...
 * - Static STAGES and CONTACTS_DATA hoisted outside component scope to prevent re-allocations on render.
 * - Sub-component ContactCard extracted and wrapped in React.memo to isolate contact list rendering.
 * - Native loading="lazy" added to images to prevent blocking render pipeline.
 * - Wrapped OpportunityDetails in React.memo to prevent re-renders when parent components update.
 */
                ...
                <div className={`rounded-xl border p-6 shadow-sm ${bgCard}`}>
                    ...
                    <div className="flex items-center justify-between relative">
                        ...
                        {STAGES.map((stage, index) => (
                            <div key={stage.name} className="relative z-10 flex flex-col items-center gap-2 bg-inherit px-2">
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="pb-6">
                                <div className="flex items-center gap-2 mb-1">
                                    <img src="https://picsum.photos/100/100?random=10" alt="John" loading="lazy" className="w-5 h-5 rounded-full" />
