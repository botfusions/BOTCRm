...
// ⚡ PERFORMANCE OPTIMIZATION: Hoisted static SQL string outside component to avoid redundant allocations
const SQL_CODE = `CREATE TABLE bots_tasks (
...
// ⚡ PERFORMANCE OPTIMIZATION: Extracted TaskItem into a memoized component to prevent redundant re-renders of the entire list
...
const Tasks: React.FC<TasksProps> = ({ darkMode }) => {
  ...
  // ⚡ PERFORMANCE OPTIMIZATION: Stabilized loadTasks with useCallback
  ...
  // ⚡ PERFORMANCE OPTIMIZATION: Stabilized toggleTask with useCallback and functional state update
  ...
  // ⚡ PERFORMANCE OPTIMIZATION: Stabilized handleAddTask with useCallback and functional state update
  ...
  // ⚡ PERFORMANCE OPTIMIZATION: Memoized theme-dependent strings
        ...
        <div className={`h-full flex flex-col items-center justify-center p-8 text-center ${darkMode ? 'bg-slate-950' : 'bg-[#F3F4F6]'}`}>
            <div className={`max-w-2xl p-10 rounded-[3rem] border shadow-2xl ${bgCard}`}>
                ...
                <div className="relative group mb-8">
                    ...
                    <button 
                        onClick={() => {navigator.clipboard.writeText(SQL_CODE); alert('SQL kopyalandı!')}}
