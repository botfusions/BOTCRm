...
// ⚡ PERFORMANCE OPTIMIZATION: Memoize TaskItem to prevent redundant re-renders when parent state (like typing in modal) changes
// ⚡ PERFORMANCE OPTIMIZATION: Hoist static strings outside of the component to prevent recreation on each render
...
}> = React.memo(({ task, darkMode, bgCard, toggleTask }) => (
...
const Tasks: React.FC<TasksProps> = ({ darkMode }) => {
  ...
  const toggleTask = useCallback(async (id: string, currentStatus: boolean) => {
    // ⚡ PERFORMANCE OPTIMIZATION: Use functional update to keep callback referentially stable
    // Optimistic update
