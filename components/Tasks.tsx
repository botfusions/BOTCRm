...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: TaskItem component is memoized to prevent
 * redundant re-renders of the entire task list when the filter or new task
 * input state changes. Re-renders are now scoped only to changed tasks.
 */
...
const Tasks: React.FC<TasksProps> = ({ darkMode }) => {
  ...
  /**
   * ⚡ PERFORMANCE OPTIMIZATION: Memoized data loader ensures that the recovery path
   * (e.g. from optimistic update failures) doesn't trigger unnecessary re-renders
   * or side effects across the application.
   */
  ...
  /**
   * ⚡ PERFORMANCE OPTIMIZATION: useCallback with functional state update pattern
   * removes the 'tasks' state from the dependency array, keeping the callback
   * referentially stable and avoiding re-renders of memoized TaskItem children.
   */
  const toggleTask = useCallback(async (id: string, currentStatus: boolean) => {
    ...
    if (!success) {
      // Recovery Path: Re-sync with server on failure
      loadTasks();
    }
  }, [loadTasks]);
